...
def run_report_markdown(run_id):
    ...
    lines = [
        ...
        f"| Same-day match candidates | {_markdown_number(row['same_day_match_candidates'])} |",
        f"| Same-day matches accepted | {_markdown_number(row['same_day_match_accepts'])} |",
        f"| Deterministic match decisions | {_markdown_number(row['matching_deterministic_decisions'])} |",
        f"| Mini match decisions | {_markdown_number(row['matching_mini_decisions'])} |",
        f"| Fail-closed match decisions | {_markdown_number(row['matching_fail_closed_decisions'])} |",
        f"| Ambiguous match cases | {_markdown_number(row['matching_ambiguous_cases'])} |",
        f"| Arc label promotions | {_markdown_number(row['story_arc_label_promotions'])} |",
        "",
        "## LLM Totals",
        "",
        "| Metric | Value |",
